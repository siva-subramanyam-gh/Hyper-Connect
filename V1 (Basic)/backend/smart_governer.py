...
#---Getting details using adb---
def get_status():
    result=subprocess.run(["adb","shell","dumpsys","battery"],capture_output=True,text=True)
    out=result.stdout
    l_match=re.search(r'level: (\d+)',out)
    t_match=re.search(r'temperature: (\d+)',out)
    level=int(l_match.group(1) if l_match else 100)
    temp=int(t_match.group(1) if t_match else 300)
    return 15.0,480

#---The Actuator(Controller)---
def set_power_save(enable:bool):
    state ="1" if enable else "0"
    subprocess.run(["adb","shell","settings","put","global","low-power",state])
    print(f"Power Saver Mode set to {enable}")
